...
# Modified version of your load_data function in main.py
def load_data(data_path: str, alg: str) -> Dict[str, Any]:
    """Load and prepare data for debiasing"""
    timer = Timer(name='data_loading')
    data_dir = Path('../MovieLens1M')
    ...
    # Load ratings - now modified to use Heater's output
    if alg == 'Heater':
        u_pref = np.load(data_dir / 'U_emb_Heater.npy')
        v_pref = np.load(data_dir / 'I_emb_Heater.npy')
